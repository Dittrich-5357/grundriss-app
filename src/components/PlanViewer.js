...
export default function PlanViewer({ plan, onBack }) {
  ...
  const analyzeGenFile = async (file) => {
    if (!file) return
      ...
      {showGenModal && (
        <div className="modal-bg" onClick={e => e.target === e.currentTarget && setShowGenModal(false)}>
          <div className="modal" style={{ width: 480 }}>
            ...
            {genKiError && (
              <div style={{ background: '#FDECEA', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#B01B0C', marginBottom: '1rem' }}>
                {genKiError} – bitte Felder manuell ausfüllen.
              </div>
            )}
